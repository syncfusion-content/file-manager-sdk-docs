---
layout: post
title: Pass a custom value to the server in Vue File Manager | Syncfusion
description: Learn how to pass a custom value to the server in the Vue File Manager for authentication, logging, or role-based access on each request.
...
# How to pass a custom value to the server in Vue File Manager
...
## 1. Setting up the Vue File Manager and provider
...
For connecting the Vue File Manager to a physical file system, check out the [`Physical file provider`](https://ej2.syncfusion.com/vue/documentation/file-manager/file-system-provider#physical-file-system-provider) section. This part of the documentation will help you configure it correctly.

## 2. Handling File Operations 

After setting the [Vue File Manager](https://www.syncfusion.com/vue-components/vue-file-manager) component with the physical file system provider, you can handle file operations by passing custom values to the server. To pass custom values during the **Read**, **Delete**, **Rename**, **Create**, **Move**, **Copy**, **Details**, **Search** and **Upload** operations, utilize the [`beforeSend`](https://ej2.syncfusion.com/vue/documentation/api/file-manager#beforesend) event of the Vue File Manager component. This event allows you to modify the request before it is sent to the server. You can add custom headers to the request to pass additional information to the server.
...
For the **GetImage** operation, use the [`beforeImageLoad`](https://ej2.syncfusion.com/vue/documentation/api/file-manager#beforeimageload) event. Inside this event, set [`useImageAsUrl`](https://ej2.syncfusion.com/vue/documentation/api/file-manager/beforeImageLoadEventArgs#useImageAsUrl) to false to instruct the Vue File Manager not to load the image directly via its URL but instead to use a fetch request. Here, attach the **Authorization** header with the value **User1** within the beforeSend event of the ajaxSettings.
